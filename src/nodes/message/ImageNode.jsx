...
import { ImageIcon } from '../../modules/projectIcons';
...
const ImageNode = (props) => {
  ...
  const [nodeName, setNodeName] = useState(data.title || 'Image Block');
  ...
  return (
    <div
      className={`media-node image-node ${selected ? 'selected' : ''}`}
    >
      ...
      <div className="media-list image-list" style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
        {data.images?.slice(0, 3).map((img, index) => (
          ...
          >
            <ImageIcon />
            <span>{img.name ? (img.name.length > 25 ? `${img.name.slice(0, 25)}...` : img.name) : `Image ${index + 1}`}</span>
          </div>
        ))}
        
        {data.images?.length > 3 && (
          <div style={{ color: '#666', fontSize: '0.8em' }}>
            + {data.images.length - 3} more images
          </div>
        )}
      </div>
