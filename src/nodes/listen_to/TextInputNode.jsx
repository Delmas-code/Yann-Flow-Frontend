...
const TextInputNode = (props) => {
  ...
  return (
    <>
      <div
        className={`media-node text-input-node ${selected ? 'selected' : ''}`}
      >
        ...
        <div className="media-list text-input-list" style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {data.textEntries?.slice(0, 3).map((entry, index) => (
            <div
              key={index}
              className="media-item text-input-item"
